...
export var accel = 20
export var defense = 0.2
onready var SWORD_BEAM = preload("res://Swordbeam/swordbeam.tscn")
...
onready var startHealth = stats.health
...
#input for the character going right
	...
	if (motion !=Vector2.ZERO):
		if motion.x>0:
			facing_right=true
			$SwordHitbox/CollisionShape2D.position.x = _animated_sprite.position.x+400
		else:
			$SwordHitbox/CollisionShape2D.position.x = _animated_sprite.position.x+500
			facing_right=false
		_animated_sprite.play("running")
		rollvector = motion * 2
		swordHitbox.knockback_vector = rollvector/3
		move()
	else:
...
func range_state():
	...
	if !attacktimer.time_left>0:
			...
			beam.knockback_vector = rollvector
...
func _on_Hurtbox_area_entered(area):
	...
	stats.health -= (area.damage-(area.damage * defense))
	print(stats.health)
