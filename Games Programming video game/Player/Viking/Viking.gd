...
#input for the character going right
	...
	if (motion !=Vector2.ZERO):
		if motion.x>0:
			...
			$Rangedhitbox/CollisionShape2D.position.x = _animated_sprite.position.x+429.84
		else:
			$SwordHitbox/CollisionShape2D.position.x = _animated_sprite.position.x+470
			$Rangedhitbox/CollisionShape2D.position.x = _animated_sprite.position.x+470
			facing_right=false
		_animated_sprite.play("running")
		rollvector = motion * 2
		swordHitbox.knockback_vector = rollvector/1.6
		move()
	else:
...
func range_state():
	...
	if !attacktimer.time_left>0:
			$Rangedhitbox/CollisionShape2D.disabled = false
			rangedHitbox.knockback_vector = rollvector/1.6
			rangedHitbox.damage=swordHitbox.damage*2
			restart_timer()
func range_animation_finished():
	$Rangedhitbox/CollisionShape2D.disabled = true
	state = MOVE
func restart_timer():
		attacktimer.set_wait_time(.8)
		attacktimer.start()
